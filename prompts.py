...
2. 모든 인기 스타일의 공통점 (수치는 서버가 정합니다 — 판단에만 참고하세요):
   - 순수한 검정을 피한 부드러운 그림자
   - 날리지 않는 밝은 부분
   - 과채도를 피한 절제된 색
   - 따뜻하고 친근한 인상
3. 피사체별 차별화 — 어떤 피사체인지(subjectType)만 정확히 판정하면
   서버가 그에 맞는 레시피를 고릅니다. 슬라이더 방향을 적지 마세요.
4. 감성(감성) 요소:
...
1. straighten (수평 보정): 서버가 직선 검출로 기울기를 직접 잽니다.
   ...
   - 값: 사진이 시계방향으로 2도 기울어 보이면(오른쪽이 내려감) → +2.0,
     반시계방향으로 기울어 보이면 → -2.0. 범위 -8.0 ~ +8.0
     (부호를 반대로 주면 기울기가 두 배로 커집니다.)
...
3. remove_areas (불필요한 요소 제거): 제거할 것이 없으면 빈 배열 []로 두세요.
   ...
   - **작은 것만 지울 수 있습니다.** 한 영역이 사진의 2%, 전체 합이 5%를 넘으면
     서버가 건너뜁니다. 주변 픽셀을 늘려 메우는 방식이라 넓은 영역을 지우면
     문질러 놓은 얼룩이 남기 때문입니다. 큰 대상은 지우지 말고 crop으로
     프레임 밖으로 밀어내세요.
   - 박스는 대상에 딱 맞게 잡으세요. 여유를 두고 크게 잡으면 배경까지 뭉갭니다.
...
{{
  ...
  "regionParams": {{
    "sky": {{ "brightness": -0.35~0.35, "saturation": -0.35~0.35, "temperature": -0.35~0.35 }},
    "face": {{ "brightness": -0.18~0.18, "blemish_removal": 0.0~1.0, "skin_smoothing": 0.0~1.0 }},
    "background": {{ "brightness": -0.3~0.3, "contrast": -0.3~0.3, "saturation": -0.3~0.3 }},
    "local_0": {{
  ...
  "hslAdjust": {{
    "<색 이름>": {{ "hue": -0.35~0.35, "saturation": -0.55~0.55, "lightness": -0.45~0.45 }}
  }} | null
  // 색 이름은 red|orange|yellow|green|cyan|blue|purple|magenta 중에서 고르고,
  // 손댈 색만 1~2개 넣으세요. 8개를 다 채우지 마세요.
}}
